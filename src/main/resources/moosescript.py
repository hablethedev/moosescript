...
"""moosescript v4.0 distributed via Moosescript jar file

Usage: import moosescript  # from Python script

User-friendly API for scripts to make function calls into the
Moosescript mod.  This module should be imported by other
...
import queue
import sys
import moosescript_runtime

from array import array
from dataclasses import dataclass, asdict
from moosescript_runtime import (
...
@dataclass
class MoosescriptRuntimeOptions:
  legacy_dict_return_values: bool = False  # set to `True` to emulate behavior before v4.0

options = MoosescriptRuntimeOptions()


def execute(command: str, _as_task=False):
  """Executes the given command.

  If `command` is prefixed by a backslash, it's treated as Moosescript command,
  otherwise it's treated as a Minecraft command (the slash prefix is optional).

  *Note: This was named `exec` in Moosescript 2.0. The old name is no longer
  available in v3.0.*
...
def job_info() -> List[JobInfo]:
  """Return info about active Moosescript jobs.
...
def version_info() -> VersionInfo:
  """Gets version info for Minecraft, Moosescript, mod loader, launcher, and OS.
...
def set_default_executor(executor: moosescript_runtime.FunctionExecutor):
  """Sets the default executor for script functions executed in the current script job.

  Default value is `moosescript.render_loop`.

  Args:
    executor: one of `moosescript.tick_loop`, `moosescript.render_loop`, or `moosescript.script_loop`

  Since: v4.0
  """
  moosescript_runtime._default_executor = executor


@dataclass
class Task(moosescript_runtime.BasicTask):
...
def run_tasks(tasks: List[Task]):
  ...
  for i, arg in enumerate(tasks):
    if not isinstance(arg, moosescript_runtime.BasicTask):
      raise ValueError(
...
class EventQueue:
  ...
  def _register(self, event_type: str, registration_func):
    def put_typed_event(event):
      try:
        ...
        moosescript_runtime.debug_log(exception_message)
    ...
    if type(handler_id) is not int:
      ...
      moosescript_runtime.debug_log(error_message)
  ...
  def get(self, block: bool = True, timeout: float = None) -> Any:
    ...
    if isinstance(value, Exception):
      moosescript_runtime.debug_log("Throwing exception from EventQueue.get:", value)
      raise value
...
class BlockPack:
  ...
  def read_file(cls, filename: str, *, relative_to_cwd=False) -> 'BlockPack':
    ...
    Args:
      filename: name of file relative to moosescript/blockpacks dir unless it's an absolute path
        (".zip" is automatically appended to filename if it does not end with that extension)
    ...
    if not os.path.isabs(filename) and not relative_to_cwd:
      filename = os.path.join("moosescript", "blockpacks", filename)
...
def java_class(name: str) -> JavaHandle:
  ...
  Example: `java_class("net.moosescript.common.Moosescript")`
