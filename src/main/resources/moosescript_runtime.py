...
"""moosescript_runtime v4.0 distributed via Moosescript jar file

Usage: import moosescript_runtime  # from Python script

Low-level interface and runtime for scripts to make function
calls into the Moosescript mod. Most users should import
moosescript.py instead for an API that is more user friendly.
"""
...
_is_debug = os.environ.get("MOOSESCRIPT_DEBUG", False) not in (False, "0", "")
if _is_debug:
  import builtins

  _debug_log = open(os.path.join("moosescript", "system", "moosescript_debug.log"), "w")
...
# See common/src/main/java/net/moosescript/common/FunctionExecutor.java
...
def call_noreturn_function(
  ...
  """
  if _exiting:
    debug_log("moosescript_runtime.py: Ignoring script function while exiting:", command, args)
    return
...
def send_script_function_request(func_name: str, args: Tuple[Any, ...],
  ...
  Args:
    func_name: name of Moosescript function to call
  ...
  if _exiting:
    debug_log("moosescript_runtime.py: Ignoring script function while exiting:", func_name, args)
    return 0
...
def _ScriptServiceLoopImpl():
  while True:
    try:
      ...
      if not json_input:
        debug_log("moosescript_runtime.py: stdin reached EOF, exiting script service loop")
        break
    ...
    if "fcid" not in reply:
      print(
          "moosescript_runtime.py: 'fcid' field missing in script function response",
          file=sys.stderr)
      continue
    ...
    if func_call_id == 0:
      if "retval" in reply:
        retval = reply["retval"]
        if retval == "exit!":
          debug_log("moosescript_runtime.py: Got `exit!` response, exiting service loop")
    ...
    with _script_function_calls_lock:
      if func_call_id not in _script_function_calls:
        print(
            f"moosescript_runtime.py: fcid={func_call_id} not found in _script_function_calls",
            file=sys.stderr)
        continue
      func_name, retval_handler, exception_handler = _script_function_calls[func_call_id]

      if "conn" in reply and reply["conn"] == "close":
        debug_log("moosescript_runtime.py: Closing connection to fcall", func_call_id)
    ...
    if "except" in reply:
      ...
      if exception_handler is None:
        ...
        debug_log("moosescript_runtime.py:", exception_message)
        print(exception_message, file=sys.stderr)
      else:
        if _is_debug:
          debug_log(
              f"moosescript_runtime.py: Passing exception to handler for fcallid {func_call_id}:",
              exception)
    ...
    if "conn" not in reply and "retval" not in reply and "except" not in reply:
      error_message = "moosescript_runtime.py: " \
...
def _ScriptServiceLoop():
  ...
  try:
    ...
    _exiting = True
    debug_log("moosescript_runtime.py: Returning from _ScriptServiceLoop")
...
def _WatchdogLoop():
  ...
  debug_log("moosescript_runtime.py: Main thread no longer alive, calling `exit!`")
